...
#define			NETWORK_TOOLS_LOOPBACK_INTERFACE_NAME			NPF_DRIVER_NAME_NORMAL " Loopback Adapter"
#define			BUF_SIZE								255

vector<string> g_InterfaceNameList1;
vector<string> g_InterfaceNameList2;

string getNetworkToolsLoopbackAdapterName()
{
...
BOOL DoRenameLoopbackNetwork2()
{
	...
	string strOriginalInterfaceName = getNetworkToolsLoopbackAdapterName();
	...
	sprintf_s(renameCmd, MAX_PATH, "netsh.exe interface set interface name=\"%s\" newname=\"%s\"", strOriginalInterfaceName.c_str(), NETWORK_TOOLS_LOOPBACK_INTERFACE_NAME);
