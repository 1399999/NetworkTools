...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	else //argc == 2
	{
		if (_tcscmp(_T("-i"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools LWF driver has failed the installation."));
				return -1;
			}
		}
		else if (_tcscmp(_T("-u"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools LWF driver has failed the uninstallation."));
				return -1;
			}
		}
		...
		else if (_tcscmp(_T("-il"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools Loopback adapter has failed the installation."));
				return -1;
			}
		}
		else if (_tcscmp(_T("-ul"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools Loopback adapter has failed the uninstallation."));
				return -1;
			}
		}
		else if (_tcscmp(_T("-iw"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools WFP callout driver has failed the installation."));
				return -1;
			}
		}
		else if (_tcscmp(_T("-uw"), argv[1]) == 0)
		{
			...
			{
				_tprintf(_T("NetworkTools WFP callout driver has failed the uninstallation."));
				return -1;
			}
		}
		else if (_tcscmp(_T("-r"), argv[1]) == 0)
		{
			...
			if (bSuccess)
			{
				_tprintf(_T("NetworkTools driver's bindings have been successfully restarted!\n"));
				return 0;
			}
			else
			{
				_tprintf(_T("NetworkTools driver's bindings have failed to restart."));
				return -1;
			}
		}
